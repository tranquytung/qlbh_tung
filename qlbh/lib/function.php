<?php 
    ...
    **/
    ...
    function base_url()
    {
        // return trả về url home của website  
        return $url  = "http://localhost:81/qlbh/";
    }

    // return trả về url admin của website 
    function public_admin()
    {
        return base_url() . "public/admin/";
    }

    // return trả về url modules của website
    function modules($url)
    {
        return base_url() . "admin/modules/" .$url ;
    }
    
    // return trả về url uploads hình ảnh của website
    function uploads()
    {
    ...
    function tinh_km($giagoc,$km)
    {
        $number = intval($giagoc);
        $khuyenmai= intval($km);
        $tinh = intval(($number * (100 - $khuyenmai))/100);
        return number_format($tinh, 0,',','.') ;
    }
...
function getLinkPagination($i){

    $queryString = $_SERVER['QUERY_STRING'];
    parse_str($queryString, $getParams);
    $getParams['page'] = $i;

    // Xây dựng lên chuỗi biến $_GET để truyền lên URL
    $newQueryString = http_build_query($getParams);
